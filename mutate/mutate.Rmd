...
- **Applications**: Some special, oft-used, applications of functions inside of mutate.</li>
...
Most (any?) functions that can be applied to an atomic vector can be applied inside of mutate.

The tidyverse package `lubridate` makes dealing with dates easy. If correctly formatted (YYYY-MM-DD), a character date field can be converted into a proper date object using the `as_date` function.

```{r, echo = TRUE, eval = TRUE}
lubridate::as_date('2021-03-15')
```

Is all lost if the date was not provided in standard format? No, but you're got a little bit more work to do. The `as_date` function has a `format =` argument where you can specify different date formats. To see the available formats see `?strptime`. Here's an example of changing a date formatted as 3/15/21 (month/day/year) to a conventional date object:

```{r, echo = TRUE, eval = TRUE}
lubridate::as_date('3/15/21', format = '%m/%d/%y')
```

**Now you** The weather `observations` data frame we created in our last lesson was pretty tidy, but the fields were not appropriately formatted. 

```{r, echo = TRUE, eval = TRUE}
glimpse(observations)
```

Using a piped operation, re-format the date field to a proper date and the weather measures (`precip`, `snow`, `temperature_min`, and `temperature_max`):

```{r observations_date_number, exercise = TRUE}

```

```{r observations_date_number-hint-1}
observations %>% 
```

```{r observations_date_number-hint-2}
observations %>% 
  mutate(date = lubridate::as_date(date)) %>% 
```

```{r observations_date_number-hint-3}
observations %>% 
  mutate(date = lubridate::as_date(date)) %>% 
  mutate_at()
```

```{r observations_date_number-hint-4}
observations %>% 
  mutate(date = lubridate::as_date(date)) %>% 
  mutate_at(
...
```{r observations_date_number-hint-5}
observations %>% 
  mutate(date = lubridate::as_date(date)) %>% 
  mutate_at(
    vars(precip:temperature_max),
...
We will deal with dates (and times) much more in-depth in the near future.

## Applications: Strings

Strings are one of the biggest wrangling challenges out there. It's also super important - I use it almost every day. The tidyverse package `stringr` makes dealing with stings pretty straightforward. We will use strings throughout the remainder of our lessons, but here is a few examples to get us started.

### Concatenating strings
The function `str_c` is used to paste together two strings. Base R has a function, `paste`, that does something similar. Unlike the base function `paste`, `str_c` includes no default separators between strings.
...
A separator between words is added manually, using the `sep =` argument:
...
### Trimming strings
The function `str_trim` removes white space around a string.
...
### Detecting strings
The function `str_detect` searches for a string in a vector. It returns `TRUE` if the string is detected and `FALSE` if it is not:

```{r, echo = TRUE, eval = TRUE}
c('goldfish', 'moldy bread', 'parsley', 'boiled') %>% 
  str_detect('old')
```
...
Two that I use often are `if_else` and `case_when` and are worth giving special here. Both perform logical tests on data and reclassify data based on that test.

For this, we will use the data frame `urban_sites`. These data, from Pharr et al. (*in prep*), provide information on the proportion of impervious surface, noise pollution, and light pollution at a series of sites within metropolitan Washington, DC.
